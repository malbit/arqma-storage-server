...
{
  static_assert(std::is_unsigned_v<T> || std::is_integral_v<T> || std::is_same_v<T, std::string_view> || std::is_same_v<T, std::string>);
  std::optional<T> res;
...
void store::load_from(json params)
{
  ...
  timestamp = std::chrono::system_clock::time_point{std::chrono::milliseconds{parse_required_stringified<uint64_t>(params, "timestamp")}};
  ...
  expiry = ttl_in ? timestamp + std::chrono::milliseconds{*ttl_in} : std::chrono::system_clock::time_point{std::chrono::milliseconds{*expiry_in}};

  auto b64_in = parse_required_field<std::string_view>(params, "data");
  if (!arqmamq::is_base64(b64_in))
    throw parse_error{"Invalid 'data' value: not base64 encoded"};

  static_assert(MAX_MESSAGE_BODY % 3 == 0, "MAX_MESSAGE_BODY should be divisible by 3 so that max base64 encoded size is exact (no padding)");
  if (b64_in.size() > MAX_MESSAGE_BODY / 3 * 4)
    throw parse_error{fmt::format("Message body exceeds maximum allowed length of {} bytes", MAX_MESSAGE_BODY)};
}
...
void retrieve::load_from(json params)
{
  ...
  if (last_hash)
  {
    if (last_hash->empty())
      last_hash.reset();
    else if (last_hash->size() != 128 || !arqmamq::is_hex(*last_hash))
...
inline const static std::unordered_set<std::string_view> allowed_arqmad_endpoints{{"get_service_nodes"sv}};

void arqmad_request::load_from(json params)
{
  auto endpoint = parse_required_field<std::string_view>(params, "endpoint");
  if (!allowed_arqmad_endpoints.count(endpoint))
    throw parse_error{fmt::format("Invalid arqmad endpoint '{}'", endpoint)};
  if (auto it = params.find("params"); it != params.end() && !it->is_null())
  {
    if (!it->is_object())
      throw parse_error{"Invalid arqmad 'params': not a dict"};
