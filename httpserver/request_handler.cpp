...
std::string computeMessageHash(const std::string& timestamp, const std::string& ttl, const std::string& recipient, const std::string& data)
{
  ...
  return arqmamq::to_hex(std::begin(hashResult), std::end(hashResult));
}
...
void RequestHandler::process_proxy_exit(const x25519_pubkey& client_key, std::string_view payload, std::function<void(arqma::Response)> cb)
{
    ...
    try {
        ...
        {
          if (req.at("headers").find(ARQMA_LONG_POLL_HEADER) != req.at("headers").end())
          {
            lp_used = req.at("headers").at(ARQMA_LONG_POLL_HEADER).get<bool>();
          }
        }
    ...
    if (lp_used)
    {
      ARQMA_LOG(debug, "Long polling requested over a proxy request");
    }
...
void RequestHandler::process_onion_to_url(const std::string& protocol, const std::string& host, uint16_t port, const std::string& target, const std::string& payload, std::function<void(arqma::Response)> cb)
{
  ...
  auto http_cb = [cb = std::move(cb)](sn_response_t res)
  {
    ...
    {
      cb(arqma::Response{Status::OK, *res.body});
    }
    else
    {
      ARQMA_LOG(debug, "Arqma server error: {}", res.error_code);
      cb(arqma::Response{Status::BAD_REQUEST, "Arqma Server error"});
    }
  };
