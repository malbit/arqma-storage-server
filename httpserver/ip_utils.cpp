...
uint32_t ipaddr_ipv4_bits(const uint8_t a, const uint8_t b, const uint8_t c, const uint8_t d)
{
  return ((a << 24) | (b << 16) | (c << 8) | d);
}

std::tuple<uint32_t, uint32_t> FromIPv4(const uint8_t a, const uint8_t b, const uint8_t c, const uint8_t d, const uint32_t netmask)
{
  return std::tuple<uint32_t, uint32_t>{ipaddr_ipv4_bits(a, b, c, d), netmask_ipv4_bits(netmask)};
}
