...
{
    arqmq_.add_category("https", arqmamq::AuthLevel::basic, 2, 1000);
...
bool HTTPSServer::check_ready(HttpResponse& res) {
    if (std::string reason; !service_node_.snode_ready(&reason)) {
        ARQMA_LOG(debug, "Storage server not ready ({}), replying with 503", reason);
    ...
    static void handle_request(
        ...
        if (auto len = req.getHeader("content-length"); !len.empty()) {
            if (uint64_t length; !util::parse_int(len, length)) {
                ARQMA_LOG(warn, "Received HTTPS request from {} with invalid Content-Length, dropping",
                ...
                ARQMA_LOG(warn, "Received HTTPS request from {} with too-large body ({} > {}), dropping",
        ...
        std::shared_ptr<call_data> data{new call_data{https, arqmq, res}};
        ...
        ARQMA_LOG(debug, "Received {} {} request from {}", req.getMethod(), request.uri, request.remote_addr);
...
void HTTPSServer::create_endpoints(uWS::SSLApp& https)
{
    ...
    {
      ARQMA_LOG(trace, "Received current https ping_test");
    ...
    {
        ARQMA_LOG(trace, "Received https ping_test");
        service_node_.update_last_ping(ReachType::HTTPS);
        Response resp{http::OK};
        resp.headers.emplace_back(http::SNODE_PUBKEY_HEADER, arqmamq::to_base64(legacy_keys_.first.hex()));
        queue_response_internal(*this, *res, std::move(resp));
    });
    ...
    https.post("/onion_req/v2", [this](HttpResponse* res, HttpRequest* req) {
        if (!check_ready(*res)) return;
        ARQMA_LOG(trace, "POST /onion_req/v2");
    ...
    https.any("/*", [this](HttpResponse* res, HttpRequest* req) {
        ARQMA_LOG(info, "Invalid HTTP request for {} {} from {}",
...
static std::variant<legacy_pubkey, Response> validate_snode_signature(ServiceNode& sn, const Request& r, bool prevalidate = false)
{
  ...
  if (!pubkey)
  {
    ARQMA_LOG(debug, "Missing or invalid pubkey header for request");
  ...
  if (auto it = r.headers.find(http::SNODE_SIGNATURE_HEADER); it != r.headers.end())
  {
    ...
    {
      ARQMA_LOG(warn, "invalid signature (not b64) found in header from {}", pubkey);
      return Response{http::BAD_REQUEST, "Invalid signature"};
    }
  }
  else
  {
    ARQMA_LOG(debug, "Missing required signature header for request");
...
void HTTPSServer::process_storage_test_req(HttpRequest& req, HttpResponse& res) {
    ...
    handle_request(*this, arqmq_, req, res, [this](std::shared_ptr<call_data> data) mutable {
        ...
        arqmq.inject_task("https", "https:" + request.uri, request.remote_addr,
            ...
            if (auto it = req.headers.find(http::SNODE_SENDER_HEADER); it != req.headers.end()) {
                if (tester_pk = parse_legacy_pubkey(it->second); !tester_pk) {
                    ARQMA_LOG(debug, "Invalid test request: invalid pubkey");
                    resp.body = "Invalid tester pubkey header";
                    return queue_response(std::move(data), std::move(resp));
                }
            } else {
                ARQMA_LOG(debug, "Invalid test request: missing pubkey");
            ...
            request_handler_.process_storage_test_req(height, tester_pk, msg_hash, [data=std::move(data), resp=std::move(resp)](MessageTestStatus status, std::string answer, std::chrono::steady_clock::duration elapsed) mutable
            {
              ...
              switch (status)
              {
                case MessageTestStatus::SUCCESS:
                  ...
                  resp.body = json{{"status", "OK"}, {"value", std::move(answer)}}.dump();
                  return queue_response(std::move(data), std::move(resp));
...
void HTTPSServer::process_storage_rpc_req(HttpRequest& req, HttpResponse& res) {
    ...
    if (addr.size() != 4) {
        ...
        ARQMA_LOG(warn, "incoming client request is not IPv4; dropping it");
        return error_response(res, http::BAD_REQUEST);
    }
    if (should_rate_limit_client(addr)) {
        ARQMA_LOG(debug, "Rate limiting client request from {}", get_remote_address(res));
        return error_response(res, http::TOO_MANY_REQUESTS);
    }

    handle_request(*this, arqmq_, req, res, [this, started=std::chrono::steady_clock::now()]
        ...
        arqmq.inject_task("https", "https:" + request.uri, request.remote_addr,
            ...
            try {
                request_handler_.process_client_req(data->request.body,
                    ...
                    ARQMA_LOG(debug, "Responding to a client request after {}",
        ...
        data->arqmq.inject_task("https", "https:" + request.uri, request.remote_addr,
            ...
            OnionRequestMetadata onion{
                ...
                [data, started](Response res) {
                    ARQMA_LOG(debug, "Got an onion response ({} {}) as edge node (after {})",
...
void HTTPSServer::shutdown(bool join)
{
    ...
    if (!sent_shutdown_)
    {
        ARQMA_LOG(trace, "initiating shutdown");
        if (!sent_startup_)
        {
        ...
        {
            loop_defer([this] {
                ARQMA_LOG(trace, "closing {} listening sockets", listen_socks_.size());
    ...
    ARQMA_LOG(trace, "joining https server thread");
    if (join)
        server_thread_.join();
    ARQMA_LOG(trace, "done shutdown");
}
