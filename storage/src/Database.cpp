...
void Database::clean_expired()
{
    ...
    sqlite3_bind_int64(delete_expired_stmt, 1, duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count());
    int rc;
    while (true) {
        ...
        if (rc == SQLITE_BUSY) {
            ...
            ARQMA_LOG(err, "Can't delete expired messages: {}", sqlite3_errmsg(db));
        }
    }
    ...
    if (reset_rc != SQLITE_OK && reset_rc != rc) {
        ARQMA_LOG(err, "sql error: unexpected value from sqlite3_reset");
    }
}
...
sqlite3_stmt* Database::prepare_statement(const std::string& query) {
    ...
    int rc = sqlite3_prepare_v2(db, query.c_str(), query.length() + 1, &stmt,
                                &pzTest);
    if (rc != SQLITE_OK) {
        ARQMA_LOG(err, "sql error: {}", pzTest);
    }
    return stmt;
}
...
static void set_page_count(sqlite3* db)
{
  ...
  int rc = sqlite3_exec(db, fmt::format("PRAGMA MAX_PAGE_COUNT = {};", Database::PAGE_LIMIT).c_str(), cb, nullptr, &errMsg);
...
void Database::open_and_prepare(const std::filesystem::path& db_path) {
    ...
    if (rc) {
        ...
        ARQMA_LOG(critical, err);
        sqlite3_close(db);
        throw std::runtime_error{err};
    }

    if (int rc = sqlite3_exec(db, "PRAGMA journal_mode = WAL", nullptr, nullptr, nullptr); rc != SQLITE_OK)
      ARQMA_LOG(err, "Failed to set journal mode to WAL: {}", sqlite3_errstr(rc));

    if (int rc = sqlite3_exec(db, "PRAGMA synchronous = NORMAL", nullptr, nullptr, nullptr); rc != SQLITE_OK)
      ARQMA_LOG(err, "Failed to set synchronous mode to NORMAL: {}", sqlite3_errstr(rc));
    ...
    delete_expired_stmt = prepare_statement("DELETE FROM Data WHERE TimeExpires <= ?");
...
bool Database::get_used_pages(uint64_t& count)
{
  ...
  while (true)
  {
    ...
    {
      ARQMA_LOG(critical, "Could not execute page count db statement");
      break;
    }
  }
  ...
  if (rc != SQLITE_OK)
  {
    ARQMA_LOG(critical, "sqlite reset error: [{}], {}", rc, sqlite3_errmsg(db));
    success = false;
  }
...
static Item extract_item(sqlite3_stmt* stmt)
{
    ...
    item.data = std::string(reinterpret_cast<const char*>(sqlite3_column_blob(stmt, 4)), sqlite3_column_bytes(stmt, 4));
    return item;
}
...
bool Database::store(std::string_view hash,
...
bool Database::bulk_store(const std::vector<Item>& items) {
    ...
    try {
        ...
        ARQMA_LOG(err, "Failed to store items during bulk operation");
    }
