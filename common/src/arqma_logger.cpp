...
public:
  void format(const spdlog::details::log_msg&, const std::tm&, spdlog::memory_buf_t& dest) override
  {
    ...
    auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - logging_t0);
    ...
    std::string elapsed;
    if (h > 0h)
      elapsed = fmt::format("{}h{:02d}m{:02d}.{:03d}s", h.count(), m.count(), s.count(), ms.count());
    else if (m > 0min)
...
void init_logging(const std::string& data_dir,
    ...
    formatter->add_flag<time_since_start_flag>('*').set_pattern("[%Y-%m-%d %H:%M:%S/+%*] [%^%l%$] [%s:%#] %v");
